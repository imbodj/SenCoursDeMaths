...
\begin{definition}
On appelle fonction  exponentielle, la bijection réciproque de la fonction $ \ln $,    notée     exp  ou e.
\end{definition}

\begin{notation}
On a donc  $ \begin{array}{lrcl}
...
\item $  \eexp{0}= 1   \; $ et $ \;    \eexp{1}= \mathrm{e} $
 ...
 \item $\forall y\in \Rr ,\; \; \ln {\eexp{y}}=y  $ 
  ...
  \item exp   est bijective et  strictement croissante  sur $\Rr$ 
...
\begin{corollary}
 ...
 \item $ \eexp{-a}=\dfrac{1}{\eexp{a}} $
  ...
  \item $ \paren{\eexp{a}}^{r} = \eexp{ra} \; \forall r \in \Qq $
 \end{itemize}
\end{corollary}
...
$ \paren{\ln^{\prime}{ \eexp{x}}}\times \text{exp}'(x)=1$ \\
$ \dfrac{\text{exp}'(x)}{\eexp{x}}=1   \Longleftrightarrow  \text{exp}'(x)=\eexp{x}$ 
...
\end{corollary}
 ...
 La fonction  $\;  u^{\prime}\eexp{u} \;$  a pour primitive  toute fonction du type  $\;  \eexp{u}+C \; $ $ (C\in\Rr) $.
...
$ \varphi $  est dérivable sur $ \intfo{0}{\pinf} \;$    et  $ \;\forall x> 0 \quad  \varphi^{\prime}(x)= \eexp{x}-1\geq 0$.  $ \;\varphi $  est donc croissante sur  $ \intfo{0}{\pinf} \;$    or  $ \;\varphi(0)=0. $  \\ Donc $ \forall x \geq 0,\; \varphi(x)\geq 0 \;$  càd  $ \eexp{x}\geq x+1 $.  Or $ \lim_{x \to \pinf}x+1=\pinf $  
...
\subsection*{ Représentation graphique de la fonction exp}
...
\item   Montrons que $ \displaystyle\lim_{x \to \pinf}\dfrac{\eexp{x}}{x}=\pinf $ \\  
 Pour $ x > 0 $,\; $ \ln  \paren{\dfrac{\eexp{x}}{x}}  =\ln \eexp{x}-\ln x  =x-\ln x  =x\paren{1-\frac{\ln x}{x}} $   d'où par produit  $\displaystyle \lim_{x \to \pinf}x\paren{1-\dfrac{\ln x}{x}}=\pinf $
 
 \item   Montrons que\; $ \lim_{x \to \minf}x\eexp{x}=0 $ \\\ 
