...
\begin{methode}[Méthode pour construire la tangente]
 Si la pente s'écrit sous la forme $f'(a)=\dfrac{\Delta y} {\Delta x}$. Alors on part de $A$, on << avance >> vers la droite du dénominateur(positif)  $ \Delta x $  et on << monte >> (ou << descend >>) du numérateur $ \Delta y $: ce qui donne un deuxième point que l'on relie à $A.$.
...
\item Si $ \displaystyle\lim_{x \to a^{+}} \dfrac{f(x)-f(a)}{x-a}=\pinf $   et $ \displaystyle\lim_{x \to a^{-}} \dfrac{f(x)-f(a)}{x-a}=\pinf $ alors la courbe de $ f $ admet  au  point $ A (a, f (a)) $ deux demi-tangentes verticales de même équation $ x=a $ l'une dirigée vers le haut et l'autre vers le bas.  A est un point d'inflexion.
...
\hline
 u'\times (v'\circ u)   &  u'\cos u  & -u'\sin u &  \dfrac{u'}{\cos^{2}u}\; \text{ou}\; u'(1+\tan^2 u)\\     
 \hline
\end{array}$
\begin{exercice}
Soit $ f (x)=\dfrac{(x+1)\sqrt{x-2}}{x-1}.$
 \begin{enumerate}
\item Justifier la continuité  de $ f $ sur son $D_{f}$.
...
Une petite variation $ \Delta x $ de la variable $ x $ provoque une petite variation  $ \Delta y = f(x+ \Delta x)-f(x) $ des images.\\ Lorsque $ \Delta x $ est voisin de $ 0 $, on assimile $ dx=\Delta x $ et on peut écrire: $ \dfrac{dy}{dx} =f'(x)$ ou $ dy=f'(x)dx $ ou $ dy=f'(x)\Delta x $.
\begin{example}
