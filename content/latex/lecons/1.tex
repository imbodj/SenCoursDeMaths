...
\begin{tikzpicture}[scale=0.375]
    ...
    \draw[red,thick,domain=0.05:3.5,samples=100] plot (\x,{(\x-1)^2 + 2});
...
\begin{tikzpicture}[scale=0.375]
    ...
    \draw[red,thick,domain=-0.01:3.2,samples=100,smooth] 
        plot (\x,{2 + (\x-2)^3});
...
$A$ est un minimum \hspace{1cm} $B$ est un maximum \hspace{1cm} $C$ est un point d'inflexion
...
\subsubsection*{Position d'une courbe par rapport à sa tangente}
  ...
  \draw[red,thick,domain=-1:\xmax,samples=100] plot (\x,{(\x)^3 - 3*(\x)^2});
