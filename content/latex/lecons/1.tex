...
\draw[red] (0.25,3) -- (0.25,-3) node[above right] {(D)};
\end{tikzpicture}
...
\draw[red] (7,2) -- (-7,2) node[below right] {D};
\end{tikzpicture}
\begin{example}
...
\draw[red] (5,5) -- (-3,-3) node[above left] {D};
\end{tikzpicture}
\begin{example}
...
\begin{tikzpicture}[scale=0.375]
    ...
    \draw[red,thick,domain=0.05:5.5,samples=100] plot (\x,{(\x-1)^2 + 2});
...
\begin{tikzpicture}[xscale=1,yscale=0.5]
    ...
    \draw[blue,thick,domain=1:7,samples=50] 
        plot (\x,{0.5*(\x-3) + 2.25});
    \node at (7.2,4.25) {tangente à $C_f$ en $A$};
...
\begin{tikzpicture}[scale=0.375]
    ...
    \draw[red,thick,domain=-2:6.5,samples=100,smooth] 
        plot (\x,{2 + (\x-2)^2});
...
\begin{tikzpicture}[scale=0.375]
    ...
    \draw[red,thick,domain=-2:3.2,samples=100,smooth] 
        plot (\x,{2 + (\x-2)^3});
...
$A$ est un minimum \hspace{1cm} $A$ est un maximum \hspace{1cm} $A$ est un point d'inflexion
...
\subsubsection*{Position d'une courbe par rapport à sa tangente}
  ...
  \end{definition}
   ...
   \begin{theorem}
   Si $ f $ est deux fois dérivable sur  un intervalle ouvert $ I $ contenant $ x_0 $  et si $ f'' $  \emph{ s'annule en changeant de signe  en $ x_0 $} alors le point de la courbe d'abscisse $ x_0 $  est un \textbf{ un point  d'inflexi
  ...
  \draw[red,thick,domain=\xmin:\xmax,samples=200] plot (\x,{(\x)^3 - 3*(\x)^2});

  % Tangente bleue : y = -3x + 1
  \draw[blue,thick,domain=-2:4,samples=100] plot (\x,{-3*\x + 1});
